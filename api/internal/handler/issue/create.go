...
func (h *Handler) CreateIssue(w http.ResponseWriter, r *http.Request) {
	...
	if issue.Title == "" || issue.Description == "" || issue.ReporterId == "" || issue.ProjectID == "" {
		httputil.WriteError(w, http.StatusBadRequest, "title, description, reporter_id, and project_id are required")
		return
	}

	if err = h.Repo.Create(&issue); err != nil {
		slog.Error("Error creating issue", "error", err)
		httputil.WriteError(w, http.StatusInternalServerError, "error creating issue")
		return
	}

	httputil.WriteJSON(w, http.StatusCreated, issue)
}
